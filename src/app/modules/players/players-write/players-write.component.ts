...
export class PlayersWriteComponent implements OnInit {
  ...
  profiles = [];
  constructor(
    public dialogRef: MatDialogRef<PlayersWriteComponent>,
  ...
  createForm(model: ICustomer): FormGroup {
    return this.formBuilder.group({
      ...
      names: [`${model.firstName} ${model.lastName}`, Validators.compose([MyValidator.required, MyValidator.minLength(4)])],
      lastName: [model.lastName, Validators.compose([MyValidator.required, MyValidator.minLength(4)])],
      username: [model.username, Validators.compose([MyValidator.required, MyValidator.minLength(4)])],
      coins: [model.player.coins, Validators.compose([MyValidator.required, MyValidator.minLength(1)])],
